...
const routes: Post[] = router.getRoutes()
  .filter(i => i.path.startsWith('/diaries') && i.meta.frontmatter.date && !i.meta.frontmatter.draft)
...
<template>
  <ul>
    ...
    <template v-for="route, idx in diaries" :key="route.path">
      <div
        v-if="!isSameGroup(route, diaries[idx - 1])"
